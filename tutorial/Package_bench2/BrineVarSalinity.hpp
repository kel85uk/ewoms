 ...
 * \tparam H2O Static polymorphism: the BrineVarSalinity class can access all properties of the H2O class
 */
template <class Scalar, class H2O>
class BrineVarSalinity : public Component<Scalar, BrineVarSalinity<Scalar, H2O> >
{
public:
    ...
    static Scalar molarMass(const Evaluation& xmlNACL)
    {
        const Scalar M1 = H2O::molarMass();
        const Scalar M2 = 58e-3; // molar mass of NaCl [kg/mol]
        const Scalar X2 = xmlNACL; // mass fraction of salt in brine
                                     ...
                                     const Evaluation& salinity)
    {
        ...
        const Evaluation& hw = H2O::liquidEnthalpy(temperature, pressure, S)/1e3; // [kJ/kg]
    ...
    static Evaluation liquidHeatCapacity(const Evaluation& temperature,
                                         const Evaluation& pressure, const Evaluation& salinity)
    {
     ...
     */
    template <class Evaluation>
    static Evaluation liquidDensity(const Evaluation& temperature, const Evaluation& pressure, const Evaluation& salinity)
    {
        Evaluation tempC = temperature - 273.15;
        Evaluation pMPa = pressure/1.0E6;
        //bool b = std::cout << "salinity = " << salinity << std::endl;
     ...
     */
    template <class Evaluation>
    static Evaluation liquidPressure(const Evaluation& temperature, const Evaluation& density, const Evaluation& salinity)
    {
        typedef MathToolbox<Evaluation> Toolbox;
        ...
        for (int i = 0;
             ...
             ++i)
        {
            const Evaluation& f = liquidDensity(temperature, pressure, salinity) - density;

            Evaluation df_dp = liquidDensity(temperature, pressure + eps, salinity);
 ...
 */
template <class Scalar, class H2O>
Scalar BrineVarSalinity<Scalar, H2O>::salinity = 0.1; // also needs to be adapted in CO2 solubility table!
